...
class HolidayController {
    ...
    async getHolidaysByUser(req, res) {
        try {
            ...
            const holidays = await HolidayRepository_1.HolidayRepository.findByUserId(userId);
    ...
    async getHolidaysByUserAndDateRange(req, res) {
        try {
            const userId = parseInt(req.params.userId);
            ...
            const holidays = await HolidayRepository_1.HolidayRepository.findByUserIdAndDateRange(userId, new Date(startDate), new Date(endDate));
            return res.json(holidays);
        }
        catch (error) {
    ...
    async createHoliday(req, res) {
        try {
            ...
            const existingHoliday = await HolidayRepository_1.HolidayRepository.findByDate(userId, new Date(date));
            if (existingHoliday) {
                return res.status(409).json({ message: 'Ya existe un festivo en esta fecha para este usuario' });
            }
            const holiday = await HolidayRepository_1.HolidayRepository.createHoliday({
                date: new Date(date),
                name,
                description,
                userId
    ...
    async createMultipleHolidays(req, res) {
        try {
            ...
            for (const holiday of holidays) {
                try {
                    ...
                    const created = await HolidayRepository_1.HolidayRepository.createHoliday({
                        date: new Date(date),
                        name,
                        description,
                        userId
    ...
    async updateHoliday(req, res) {
        try {
            ...
            const holiday = await HolidayRepository_1.HolidayRepository.findOneBy({ id: holidayId });
            if (!holiday) {
            ...
            const updated = await HolidayRepository_1.HolidayRepository.save(holiday);
    ...
    async deleteHoliday(req, res) {
        try {
            ...
            await HolidayRepository_1.HolidayRepository.delete(holidayId);
    ...
    async deleteAllHolidaysByUser(req, res) {
        try {
            const userId = parseInt(req.params.userId);
            if (isNaN(userId)) {
                return res.status(400).json({ message: 'ID de usuario inválido' });
            }
            await HolidayRepository_1.HolidayRepository.delete({ userId });
